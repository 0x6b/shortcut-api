...
/// StorySlim : StorySlim represents the same resource as a Story, but is more light-weight. For
/// certain fields it provides ids rather than full resources (e.g., `comment_ids` and `file_ids`)
/// and it also excludes certain aggregate values (e.g., `cycle_time`). The `description` field can
/// be optionally included. Use the [Get Story](#Get-Story) endpoint to fetch the unabridged payload
/// for a Story.
...
impl StorySlim {
    /// StorySlim represents the same resource as a Story, but is more light-weight. For certain
    /// fields it provides ids rather than full resources (e.g., `comment_ids` and `file_ids`) and
    /// it also excludes certain aggregate values (e.g., `cycle_time`). The `description` field can
    /// be optionally included. Use the [Get Story](#Get-Story) endpoint to fetch the unabridged
    /// payload for a Story.
    pub fn new(
        app_url: String,
        archived: bool,
